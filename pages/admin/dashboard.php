...
// Paid Routes (Step 6)
$paid_routes = $pdo->query("SELECT r.*, u.name as user_name, u.cpf as user_cpf FROM routes r JOIN users u ON r.user_id = u.id WHERE r.wizard_step = 6 AND r.is_archived = 0 ORDER BY r.completed_at DESC")->fetchAll();

// Active routes include 'assigned', 'accepted', 'in_progress' and 'delayed'
$active_routes = $pdo->query("SELECT r.*, u.name as user_name, u.microregion as user_macroregion FROM routes r JOIN users u ON r.user_id = u.id WHERE r.status IN ('pending_acceptance', 'accepted', 'in_progress', 'delayed') AND (r.scheduled_end IS NULL OR r.scheduled_end >= NOW()) AND r.is_archived = 0 ORDER BY r.created_at DESC")->fetchAll();

// Expired routes (active and past deadline)
$expired_routes = $pdo->query("SELECT r.*, u.name as user_name, u.microregion as user_macroregion FROM routes r JOIN users u ON r.user_id = u.id WHERE r.status IN ('pending_acceptance', 'accepted', 'in_progress', 'delayed') AND r.scheduled_end IS NOT NULL AND r.scheduled_end < NOW() AND r.is_archived = 0 ORDER BY r.scheduled_end ASC")->fetchAll();

// Completed routes - Todas as que foram entregues mas AINDA NÃO foram pagas (Passo < 6)
$completed_routes = $pdo->query("SELECT r.*, u.name as user_name FROM routes r JOIN users u ON r.user_id = u.id WHERE r.status = 'completed' AND (r.wizard_step < 6 OR r.wizard_step IS NULL) AND r.is_archived = 0 ORDER BY r.completed_at DESC")->fetchAll();

// Rotas para a Calculadora: wizard_step >= 3, qualquer status (incluindo pagas para consulta)
$calc_routes = $pdo->query("SELECT r.*, u.name as user_name, u.processo_sei FROM routes r JOIN users u ON r.user_id = u.id WHERE r.wizard_step >= 4 AND r.is_archived = 0 ORDER BY r.completed_at DESC, r.created_at DESC")->fetchAll();

// Cancelled routes
$cancelled_routes = $pdo->query("SELECT r.*, u.name as user_name FROM routes r JOIN users u ON r.user_id = u.id WHERE r.status = 'cancelled' AND r.is_archived = 0 ORDER BY r.updated_at DESC")->fetchAll();

// Rejected routes
$rejected_routes = $pdo->query("SELECT r.*, u.name as user_name FROM routes r JOIN users u ON r.user_id = u.id WHERE r.status = 'rejected' AND r.is_archived = 0 ORDER BY r.created_at DESC")->fetchAll();

// Archived routes
$archived_routes = $pdo->query("SELECT r.*, u.name as user_name, u.microregion as user_macroregion FROM routes r JOIN users u ON r.user_id = u.id WHERE r.is_archived = 1 ORDER BY r.created_at DESC")->fetchAll();
...
$report_sql = "
    ...
    FROM users u
    LEFT JOIN routes r ON u.id = r.user_id AND r.is_archived = 0
...
$micro_routes_count = $pdo->query("SELECT microregion, COUNT(*) as total FROM routes WHERE status NOT IN ('cancelled', 'rejected') AND is_archived = 0 GROUP BY microregion")->fetchAll();
$macro_report = [];
...
<body>
    ...
    <div class="admin-layout">
        ...
        <main class="main-content">
            ...
            <div id="archived" class="tab-content" style="display: none;">
                ...
                <?php if (count($archived_routes) > 0): ?>
                    <div class="grid grid-2">
                        <?php foreach ($archived_routes as $route): ?>
                            <div class="monitor-card <?php echo ($route['status'] == 'delayed') ? 'delayed' : ''; ?>">
                                <!-- Card Header with Status & Actions -->
                                <div class="card-header-actions">
                                    <div>
    ...
    <script>
        ...
        function openCancelModal(routeId, routeTitle) {
            document.getElementById('cancel-route-id').value = routeId;
            document.getElementById('cancel-modal-route-title').innerText = routeTitle;
            document.getElementById('cancel-reason-input').value = '';
            document.getElementById('cancel-route-modal').style.display = 'flex';
        }
